...
namespace Unity.FPS.Gameplay
{
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        {
            return ControlFreak2.CFCursor.lockState == CursorLockMode.Locked && !m_GameFlowManager.GameIsEnding;
        }
        ...
        public Vector3 GetMoveInput()
        {
            if (CanProcessInput())
            {
                Vector3 move = new Vector3(ControlFreak2.CF2Input.GetAxisRaw(GameConstants.k_AxisNameHorizontal), 0f,
                    ControlFreak2.CF2Input.GetAxisRaw(GameConstants.k_AxisNameVertical));
        ...
        public bool GetAimInputHeld()
        {
            if (CanProcessInput()||true)
            {
                ...
                //bool i = isGamepad
                //    ? (ControlFreak2.CF2Input.GetAxis(GameConstants.k_ButtonNameGamepadAim) > 0f)
                //    : (ControlFreak2.CF2Input.GetAxis(GameConstants.k_ButtonNameAim)>0f);
                //return i;

                Debug.Log("TEST");
                if((Input.GetAxis(GameConstants.k_ButtonNameAim) > 0f))
                {
                    Debug.Log("TEST-TRUE");
                    return true;
                }
        ...
        public bool GetReloadButtonDown()
        {
            if (CanProcessInput())
            {
                return ControlFreak2.CF2Input.GetButtonDown(GameConstants.k_ButtonReload);
            }
        ...
        float GetMouseOrStickLookAxis(string mouseInputName, string stickInputName)
        {
            if (CanProcessInput())
            {
                ...
                if (InvertYAxis && (stickInputName== "Vertical" || mouseInputName== "Mouse Y"))
                    i *= -1f;
